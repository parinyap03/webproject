...
const TeacherDetails = () => {
  ...
  useEffect(() => {
    const q = query(collection(doc(db, "checkin", id), "questions"));
  ...
  const deleteQuestion = async (questionId) => {
    const questionDocRef = doc(db, "checkin", id, "questions", questionId);
    await deleteDoc(questionDocRef);
  };

  useEffect(() => {
    const fetchAnswers = async () => {
      const answersCollectionRef = collection(db, "checkin", id, "questions");
    ...
    <div>
      ...
      <div className="flex flex-col md:flex-row justify-between mt-5">
        ...
        <div className="w-full md:w-1/2 p-5 m-2 bg-white rounded shadow-md mt-5 md:mt-0">
          <h2 className="text-xl font-bold">All Answer</h2>
