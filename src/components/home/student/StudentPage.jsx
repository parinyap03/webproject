...
const Student = () => {
  ...
  useEffect(() => {
    ...
    const q = query(collection(doc(db, "checkin", checkInId), "questions"));
  ...
  const handleSubmitAns = (questionId) => async (e) => {
    ...
    const processSubmission = async () => {
      try {
        ...
        await updateDoc(
          doc(db, "checkin", checkInId, "questions", questionId),
          {
        ...
        <div className="w-full p-4 bg-white shadow-md rounded-md">
          <form onSubmit={handleCheckIn}>
            <input
              ...
              placeholder="Enter ID to check in"
            />
            <button
              type="submit"
